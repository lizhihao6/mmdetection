...
class ResBlock(nn.Module):
    def __init__(self, channel_size: int, negative_slope: float = 0.2):
        ...
        _, norm1 = build_norm_layer(norm_cfg, channel_size, postfix=1)
        _, norm2 = build_norm_layer(norm_cfg, channel_size, postfix=2)
...
class Resizer(nn.Module):
    def __init__(self, in_ch=4, num_kernels=16, num_resblocks=2, negative_slope=0.2):
        super().__init__()
        n, r, slope = num_kernels, num_resblocks, negative_slope
        norm_cfg = dict(type='SyncBN', requires_grad=True)
        _, norm1 = build_norm_layer(norm_cfg, n, postfix=1)
        _, norm2 = build_norm_layer(norm_cfg, n, postfix=2)

        self.module1 = nn.Sequential(
            nn.Conv2d(in_ch, n, kernel_size=7, padding=3),
        ...
        self.module4 = nn.Conv2d(n, in_ch, kernel_size=7,
                                 padding=3)
...
class MobileNetV2RAW(BaseModule):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(MobileNetV2RAW, self).__init__(init_cfg)
        ...
        if use_resizer:
            self.resizer = Resizer(in_ch=in_ch)
    ...
    def train(self, mode=True):
        ...
        frozen."""
        super(MobileNetV2RAW, self).train(mode)
        self._freeze_stages()
