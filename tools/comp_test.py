...
def parse_args():
    ...
    if args.comp_checkpoint == 'None':
        args.comp_checkpoint = None
    return args
...
def single_gpu_test(model,
    ...
    bpp, psnr, counter = 0., 0., 0
    for i, data in enumerate(data_loader):
        bpp += float(data.pop('bpp'))
        psnr += float(data.pop('psnr'))
        counter += 1
        with torch.no_grad():
        ...
        for _ in range(batch_size):
            prog_bar.update()
    print(f'\nCompression results: bpp {bpp/counter}, psnr {psnr/counter}')
    return results
...
def main():
    ...
    cfg.data.test['type'] = 'MultiRAWCompDataset'
    cfg.data.test['load_pipeline'] = [cfg.data.test['pipeline'][0]]
